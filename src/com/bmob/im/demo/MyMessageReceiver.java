...
public class MyMessageReceiver extends BroadcastReceiver {
					...
					if(TextUtils.isEmpty(tag)){//不携带tag标签
						...
						if(tag.equals(BmobConfig.TAG_ADD_CONTACT)){
							...
							if(toId.equals(currentUser.getObjectId())){
								if (ehList.size() > 0) {// 有监听的时候，传递下去
									...
									if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
										...
										BmobNotifyManager.getInstance(context).showNotify(isAllow, isAllow, R.drawable.ic_launcher, tickerText, message.getFromname(), tickerText.toString(),NewFriendActivity.class);
									}
								}
							}
							...
							if(isAllow && currentUser!=null && currentUser.getObjectId().equals(toId)){
								...
								BmobNotifyManager.getInstance(context).showNotify(isAllow, isAllow, R.drawable.ic_launcher, tickerText, username, tickerText.toString(),ChatMainActivity.class);
							}
	...
	public void showNotification(Context context,BmobMsg msg) {
		...
		BmobNotifyManager.getInstance(context).showNotifyWithExtras(false, false, icon, tickerText.toString(), contentTitle, tickerText.toString(),intent);
